...
def inscribirse(request, taller_id):
    ...
    if InscripcionTaller.objects.filter(usuario=adulto_mayor, taller=taller.codigo).exists():
...
@login_required
def eliminar_taller(request, inscripcion_id):
    inscripcion = get_object_or_404(InscripcionTaller, id=inscripcion_id)

    # Restaurar el cupo del taller
